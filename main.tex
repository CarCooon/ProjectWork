...
% --- Impaginazione ---
% Margini ridotti rispetto alla versione precedente (era margin=3cm,
% bindingoffset=0.5cm) per contenere il numero di pagine: 2.2cm resta un
% margine standard e pienamente leggibile per un elaborato accademico,
% senza il bindingoffset (non necessario per un documento non rilegato
% fisicamente).
\usepackage[margin=2.2cm]{geometry}
\usepackage{setspace}
% Interlinea ridotta da 1.2 a 1.05: ancora comoda da leggere, ma
% sensibilmente più compatta della precedente (1.2 è vicina a una
% "1.5 righe" in termini di elaboratore di testo).
\setstretch{1.05}
...
\usepackage{colortbl}    % colori nelle celle delle tabelle
\usepackage{makecell}    % \makecell per line-break in celle di tabella
...
\begin{center}
    ...
    % Università
    {\Huge Università degli Studi di Salerno}\\[8mm]
    ...
    % Logo
    \includegraphics[width=0.30\textwidth]{immagini/logo_unisa}\\[15mm]

    % Corso
    {\LARGE Algoritmi e Protocolli per la Sicurezza}\\[24mm]

    % Tipo elaborato
    {\Huge\bfseries PROJECT WORK}\\[17mm]
...
\begin{center}
    ...
    \begin{tabular}{c @{\hspace{2cm}} c}
        \textbf{\Large Carmine Terracciano} &
        \textbf{\Large Ferdinando Paparo} \\[3mm]

        \large Matr. IE22700109 &
        \large Matr. IE22700107
    \end{tabular}
...
% Anno Accademico
\begin{center}
    {\huge Anno Accademico 2025/2026}
\end{center}
...
%
% Il corpo dell'indice resta a grandezza piena (invariato). Si riduce invece
% solo lo spazio intorno al titolo "Sommario" (che \tableofcontents genera
% internamente come \chapter* con \@makeschapterhead): di default lo standard
% "book" lascia 50pt sopra e 40pt sotto il titolo, più del necessario per
% recuperare la sola riga che eccedeva. Ridotti a 22pt/18pt, e il titolo
% stesso di un mezzo passo (da \Huge a \huge) per accompagnare la riduzione
% dello spazio circostante in modo proporzionato.
\makeatletter
...
% \clearpage invece di \cleardoublepage: i capitoli iniziano sulla pagina
% successiva senza il vincolo di essere una pagina dispari, evitando le
% pagine bianche di riempimento (la classe è openany, quindi non c'è un
% vincolo strutturale a partire da pagina destra).
\clearpage
\input{capitoli/wp1}
