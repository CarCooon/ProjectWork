...
La sicurezza di un sistema di voto elettronico non è mai assoluta, ma dipende da alcune ipotesi sul comportamento di chi vi partecipa e sull'ambiente in cui si vota. Di seguito analizziamo queste assunzioni di fiducia, spiegando perché le abbiamo scelte e cosa succede se non vengono rispettate. In generale, il sistema garantisce la segretezza, l'integrità e la verificabilità del voto a condizione che si verifichino le seguenti assunzioni.

\subsection*{1. Non collusione tra Sistema di Autenticazione e Autorità Elettorale}
\textbf{Dichiarazione:} Si assume che il Sistema di Autenticazione (SA) e l'Autorità Elettorale (AE) lavorino in modo del tutto indipendente, senza scambiarsi informazioni di nascosto. \\
\textbf{Motivazione e compromessi:} Il sistema garantisce lo pseudoanonimato a condizione che questa separazione venga rispettata. Il SA sa chi vota, mentre l'AE sa cosa viene votato. Tenerli separati è un compromesso pratico per avere un sistema leggero e funzionante, fidandoci del fatto che i due gestori non si accordino per imbrogliare. \\
\textbf{Rischio Residuo:} Se questa assunzione crolla e le due entità comunicano, si perde totalmente lo pseudoanonimato. Gli amministratori potrebbero facilmente incrociare i dati di accesso con l'arrivo delle schede, scoprendo chi ha votato cosa.
...
\textbf{Motivazione e compromessi:} Il sistema garantisce la segretezza e la correttezza del singolo voto a condizione che il dispositivo dell'utente sia "pulito". Votando a distanza non abbiamo cabine elettorali fisiche, quindi dobbiamo per forza delegare la sicurezza iniziale al dispositivo dell'elettore. Purtroppo, il sistema non può difendersi da minacce che agiscono direttamente sullo schermo o sulla tastiera dell'utente. \\
\textbf{Rischio Residuo:} Se il dispositivo è infetto, un virus può leggere la scelta prima ancora che venga protetta, violando la segretezza. Nel peggiore dei casi, potrebbe persino inviare all'urna una preferenza diversa da quella cliccata dall'elettore.
...
\textbf{Rischio Residuo:} Non potendo cambiare la scelta in un secondo momento, se qualcuno costringe con la forza un elettore a votare davanti a lui, il ricatto va a buon fine. Il rischio residuo è la totale perdita di libertà del votante, un limite molto difficile da superare nei sistemi di voto a distanza.

\subsection*{5. Sicurezza della crittografia di base}
\textbf{Dichiarazione:} Si assume che gli algoritmi crittografici scelti siano sicuri e che nessun attaccante abbia a disposizione una potenza di calcolo sufficiente per romperli in tempi brevi. \\
