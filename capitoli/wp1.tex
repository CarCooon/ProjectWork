...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[actor] (elettore) at (0,0) {
            \textbf{Elettore}\\[-1pt]
            \footnotesize seleziona e invia la scheda
        };

        \node[actor, left=3.35cm of elettore] (sa) {
            \textbf{SA}\\[-1pt]
            \footnotesize Sistema di Autenticazione
        };

        \node[actor, right=3.35cm of elettore] (ae) {
            \textbf{AE}\\[-1pt]
            \footnotesize Autorità Elettorale
        };

        \node[storage, below=1.7cm of ae] (urna) {
            \textbf{Urna / Bacheca}\\[-1pt]
            \footnotesize record pubblici
        };

        \node[actor, below=1.7cm of elettore] (vp) {
            \textbf{Verificatore}\\[-1pt]
            \footnotesize controlla dati e prove
        };
        ...
        \begin{scope}[on background layer]
            \node[
                trustarea,
                fit=(sa),
                label={[font=\footnotesize, text=black!55]above:Dominio identità}
            ] {};

            \node[
                trustarea,
                fit=(ae)(urna),
                label={[font=\footnotesize, text=black!55]above:Dominio voto}
            ] {};
        \end{scope}

        \draw[arr, bend left=18]
            (elettore.north west) to
            node[lbl, above] {richiesta di accesso}
            (sa.north east);

        \draw[arr, bend left=18]
            (sa.south east) to
            node[lbl, below] {abilitazione pseudonima}
            (elettore.south west);

        \draw[arr]
            (elettore) --
            node[lbl, above] {scheda protetta}
            (ae);

        \draw[arr]
            (ae) --
            node[lbl, right] {registrazione}
            (urna);

        \draw[arr]
            (urna) --
            node[lbl, below] {dati verificabili}
            (vp);

        \draw[dashed, black!45, line width=0.9pt]
            (sa.north east)
            .. controls ($(sa.north)+(2.4,0.8)$) and ($(ae.north)+(-2.4,0.8)$) ..
            node[lbl, above] {$\times$ separazione SA/AE}
            (ae.north west);
        ...
        \node[
            ...
            gestisce le schede, non le identità reali
        };

    \end{tikzpicture}

    \caption{Architettura logica degli attori e separazione dei flussi informativi: il SA gestisce l'identità e l'abilitazione, mentre l'AE gestisce la raccolta delle schede.}
